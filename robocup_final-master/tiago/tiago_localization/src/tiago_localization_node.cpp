...
namespace tiago_localization
{
    ...
    bool Localizer::initialize(ros::NodeHandle& nh)
    {
        ...
        pose_sub_ =  nh.subscribe("/amcl_pose",1000, &Localizer::poseCallback,this);
        //#>>>> Hint: see http://wiki.ros.org/amcl

        //#>>>>TODO: advertise a geometry_msgs::Twist to command the robot by velocity
        vel_pub_ = nh.advertise<geometry_msgs::Twist>("/mobile_base_controller/cmd_vel", 1000);
        ...
        if (localization_client_.call(srv/*#>>>>TODO: YOUR SRV NAME*/)) {
    ...
    void Localizer::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
        ...
        covariance_sum = 0;
        if ((current_time - timestamp).toSec() <= 0.5/*#>>>>TODO: CHECK THE INTERVAL IS WITHIN 0.5 SECOND*/){
