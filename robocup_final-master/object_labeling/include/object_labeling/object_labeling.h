...
private:
  bool labelObjects(CloudPtr& input, project_msgs::LabeledCentroid& output, CloudPtr& remainders);
...
private:
  ...
  ros::Subscriber table_point_cloud_sub_;
  ros::Subscriber camera_info_sub_;         //!< sub camera info
  ros::Subscriber execute_sub_;             //!< execute once when sub 
  ros::Publisher labeled_object_centroid_pub_; //!< publisher for labeled pointcloud
  ros::Publisher labeled_remainder_cloud_pub_; //!< publisher for labeled pointcloud
  ...
  project_msgs::CollisionGeometry collision_objects;

  //visualization_msgs::MarkerArray text_markers_;  //!< text markers for rviz
  CloudPtr remaind_point_cloud_;
  // inputs 
  CloudPtr object_point_cloud_;                             //!< objects point cloud
  CloudPtr table_point_cloud_;                             //!< objects point cloud1
